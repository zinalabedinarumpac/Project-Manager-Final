...
def main():
    ...
    def filled_table():
        ...
        conn = sqlite3.connect('Memberlist.db')
        c = conn.cursor()
        c.execute("""SELECT m.ID_NUMBER, m.NAME, m.STATUS, m.TASK, p.PROJECT_CODE, p.PROJECT, t.TASK_STATUS
                      FROM memberlist AS m
                      INNER JOIN projectlist AS p ON p.PROJECT_CODE = m.PROJECT_CODE
                      INNER JOIN tasklist AS t ON t.TASK_NAME = m.TASK""")
    ...
    def reg_task():
        ...
        c.execute("INSERT INTO tasklist VALUES(:TASK_ID, :TASK_NAME, :TASK_STATUS)",
                  {
                      'TASK_ID': reg_task_id.get(),
        ...
        task_id['values'] = tclist()
        reg_taskstatus['values'] = ("Not Yet Started", "In Progress", "Done")  # Update reg_taskstatus values

        reg_task_id.delete(0, END)
        reg_taskname.delete(0, END)
        reg_taskstatus.set("SELECT STATUS")  # Reset reg_taskstatus combobox
    ...
    def task_list():
        ...
        tree1 = ttk.Treeview(frm, columns=(1, 2, 3), show="headings", height=9)
        tree1.pack()

        tree1.heading(1, text="TASK ID", anchor=CENTER)
        tree1.column("1", minwidth=0, width=120)
        tree1.heading(2, text="TASK NAME", anchor=CENTER)
        tree1.column("2", minwidth=0, width=125)
        tree1.heading(3, text="TASK STATUS", anchor=CENTER)
        tree1.column("3", minwidth=0, width=125)
    ...
    # TASK FORM
    L6 = Label(root, text="TASK ID:", bg="maroon", fg="white", font=("Lucida Console", 8, "bold"))
    L6.place(x=1080, y=15)
    L7 = Label(root, text="TASK NAME:", bg="maroon", fg="white", font=("Lucida Console", 8, "bold"))
    L7.place(x=1080, y=55)
    L8 = Label(root, text="STATUS:", bg="maroon", fg="white", font=("Lucida Console", 8, "bold"))
    L8.place(x=1080, y=95)
    ...
    reg_task_id = Entry(root, bd=2, width=27, font=("Lucida Console", 8))
    reg_task_id.place(x=1080, y=35)
    reg_taskname = Entry(root, bd=2, width=27, font=("Lucida Console", 8))
    reg_taskname.place(x=1080, y=75)
    ...
    reg_taskstatus.place(x=1080, y=115)
